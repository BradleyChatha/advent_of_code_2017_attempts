...
final class Grid
{
	public
	{
		...
		void move(Direction direction, bool doPause = false)
		{
			if(steps.length == 0)
			{
				//writeln("Adding: ", direction);
				this.steps ~= direction;
				return;
			}

			void rewrite(Direction[] stepsToRemove, Direction[] stepsToRewriteAs, Direction stepToAddIfNoRewrite)
			{
				//writeln("Direction: ", stepToAddIfNoRewrite);

				// Find if there's something we need to remove.
				auto removeIndex = -1;
				auto rewriteDirection = Direction.None;
				for(int i = this.steps.length - 1; i > -1; i--)
				{
					foreach(i2, removeDir; stepsToRemove)
					{
						if(this.steps[i] == removeDir)
						{
							removeIndex = i;
							rewriteDirection = stepsToRewriteAs[i2];
							break;
						}
					}
				...
				// If we don't need to rewrite, add the right step in.
				if(removeIndex == -1 && stepToAddIfNoRewrite != Direction.None)
				{
					//writeln("[No Rewrite] Adding: ", stepToAddIfNoRewrite);
					this.steps ~= stepToAddIfNoRewrite;

					if(this.steps.length > this.furthestDistance)
						this.furthestDistance = this.steps.length;
				}
				else
				{
					// Otherwise, remove the step, and add the other step. (10/10 comments)
					//writeln("[Rewrite] Removing ", this.steps[removeIndex], " from position ", removeIndex);
					foreach(i; removeIndex..this.steps.length)
					{
					...
					if(rewriteDirection != Direction.None)
					{
						//writeln("[Rewrite] Moving in: ", rewriteDirection);
						this.move(rewriteDirection);
					}
				}

				//writeln("Steps(", this.steps.length, "): ", this.steps, "\n");
				if(doPause)
					readln();
			}
			...
			final switch(direction) with(Direction)
			{
				...
				case North: rewrite([SouthEast, South, SouthWest], [NorthEast, None, NorthWest], North); break;
				case South: rewrite([NorthEast, North, NorthWest], [SouthEast, None, SouthWest], South); break;
				case East:  assert(false); 
				case West:  assert(false);

				case NorthEast: rewrite([NorthWest, SouthWest, South], [North, None, SouthEast], NorthEast); break;
				case SouthEast: rewrite([SouthWest, NorthWest, North], [South, None, NorthEast], SouthEast); break;
				case SouthWest: rewrite([SouthEast, NorthEast, North], [South, None, NorthWest], SouthWest); break;
				case NorthWest: rewrite([NorthEast, SouthEast, South], [North, None, SouthWest], NorthWest); break;
			}
		}

		int getStepsFromStart()
		{
			writeln("Steps: ", this.steps);

			return this.steps.length;
		}

		int getFurthestDistance()
		{
...
void main()
{
	auto grid = parseString(INPUT, false);
	writeln("Steps: ", grid.getStepsFromStart());
	writeln("Furthest distance: ", grid.getFurthestDistance());
}

Grid parseString(string input, bool doPause = false)
{
	...
	foreach(dirString; input.splitter(","))
	{
		auto direction = directions.get(dirString, Direction.None);
		assert(direction != Direction.None, dirString);

		grid.move(direction, doPause);
	}
...
{
	auto grid = new Grid();
	grid.move(Direction.NorthEast);
	grid.move(Direction.NorthEast);
	grid.move(Direction.NorthEast);

	assert(grid.getStepsFromStart() == 3);
}
...
unittest
{
	...
	grid.move(Direction.NorthEast);
	grid.move(Direction.NorthEast);
	grid.move(Direction.SouthWest);
	grid.move(Direction.SouthWest);

	assert(grid.getStepsFromStart() == 0);
}
...
unittest
{
	...
	assert(grid.getStepsFromStart() == 2);
}
...
unittest
{
	...
	auto grid = new Grid();
	grid.move(Direction.SouthEast); // 1 1
	grid.move(Direction.SouthWest); // 2 0
	grid.move(Direction.SouthEast); // 3 1
	grid.move(Direction.SouthWest); // 4 0
	grid.move(Direction.SouthWest); // 5 -1 (1)

	assert(grid.getStepsFromStart() == 3, format("%s", grid.getStepsFromStart()));
}

unittest
{
	auto grid = parseString("se,sw,se,sw,sw");
	assert(grid.getStepsFromStart() == 3);
}

unittest
{
	auto grid = parseString("n,n,nw,ne,n,sw");
	assert(grid.getStepsFromStart() == 4);
}

unittest
{
	auto grid = parseString("n,n,nw,ne,n,se");
	assert(grid.getStepsFromStart() == 4);
}

unittest
{
	auto grid = parseString("s,s,se,sw,s,ne");
	assert(grid.getStepsFromStart() == 4);
}

unittest
{
	void testSet(string input, int stepCount)
	{
		auto grid = parseString(input);
		assert(grid.getStepsFromStart() == stepCount, input);
	}
