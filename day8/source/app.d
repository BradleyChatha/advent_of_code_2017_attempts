...
const INPUT = "ebu inc 626 if iq < 0
...
const REGEX = `([a-z]+)\s*(inc|dec)\s*([-\d]+)\s*if\s([a-z]+)\s([><=!]+)\s([-\d]+)`;
// [1] = register to modify.
// [2] = inc/dec
// [3] = number to inc/dec by
// [4] = condition register
// [5] = condition (>=, !=, etc.)
// [6] = condition number

alias CompareFunc = bool function(int x, int y);

void main()
{
	auto partTwoValue = 0;
	auto registers = INPUT.execute(partTwoValue);
	auto maxValue = 0;
	foreach(register, value; registers)
	{
		if(value > maxValue)
			maxValue = value;
	}

	writeln("Part One: ", maxValue);
	writeln("Part Two: ", partTwoValue);
}

int[string] execute(const(char)[] input, out int properMaxValue)
{
	import std.regex;
	import std.algorithm : splitter, map;
	import std.conv : to;

	int[string] registers = ["SUPERNOVADUMMY": 0];
	registers.remove("SUPERNOVADUMMY");

	auto reg = ctRegex!REGEX;
	foreach(lineRegex; input.splitter('\n').map!(l => l.matchFirst(reg)))
	{
		auto target = lineRegex[1];
		auto action = lineRegex[2];
		auto targetNumber = lineRegex[3].to!int;
		auto conditionTarget = lineRegex[4];
		auto conditionAction = lineRegex[5];
		auto conditionNumber = lineRegex[6].to!int;

		if((target in registers) is null)
			registers[cast(string)target] = 0;

		if((conditionTarget in registers) is null)
			registers[cast(string)conditionTarget] = 0;

		auto comparer = conditionAction.toCompareFunc();
		if(comparer(registers[conditionTarget], conditionNumber))
		{
			if(action == "inc")
				registers[target] += targetNumber;
			else if(action == "dec")
				registers[target] -= targetNumber;
			else
				assert(false, action);
		...
		if(registers[target] > properMaxValue)
			properMaxValue = registers[target]; 
	}

	return registers;
}

CompareFunc toCompareFunc(const(char)[] action)
{
	switch(action)
	{
		case "==": return (x, y) => x == y;
