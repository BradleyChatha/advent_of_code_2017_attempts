...
void main()
{
    writeln(INPUT.countValid);
}

int countValid(const(char)[] input)
{
    import std.algorithm : splitter, count, canFind, any;
    ...
    {
        writeln("Current line: ", line);

        string[] words;
        bool failed = false;
        foreach(word; line.splitter!isWhite)
        {
            writeln("\tCurrent Word: ", word);
            if(words.canFind(word))
            {
                writeln("\t\tWord is duplicate");
                failed = true;
                break;
            }
            else
            {
                ...
                {
                    if(otherWord.isAnagram(word))
                    {
                        writeln("\t\tWord is anagram of ", otherWord);
                        failed = true;
                    }
                }
        ...
        {
            total += 1;
            writeln("Increased total to ", total);
        }
    }
...
bool isAnagram(const(char)[] original, const(char)[] toTest)
{
    ...
    int[char] originalCount;
    int[char] toTestCount;

    void count(ref int[char] countings, const(char)[] input)
    {
        foreach(ch; input)
        {
            if((ch in countings) is null)
                countings[ch] = 0;
    ...
    count(originalCount, original);
    count(toTestCount, toTest);

    foreach(k, v; toTestCount)
    {
        if((k in originalCount) is null || originalCount[k] != v)
            return false;        
    }
